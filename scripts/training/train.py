...
train_params = {    
    ...
    "paths": {
        ...
        "model": """/autofs/cluster/octdata2/users/epc28/veritas/output/models/version_5""",
    },
    "data": {
        "n_volumes": 1000,
        "shape": 256,
...
class SetUp(object):
    ...
    def model(self, train_new: bool=True):
        ...
        if train_new == True:
            ...
            self.FTtrainee = FineTunedTrainee.load_from_checkpoint(
                checkpoint_path=best_checkpoint, trainee=self.trainee,
                loss=self.losses
                )
    ...
    def trainer(self, trainee, train_loader, val_loader, segnet, 
        ...
        if self.gpus == 1:
            ...
            trainer = Trainer(accelerator=device, benchmark=True, 
                default_root_dir=train_params['paths']['vesselseg_dir'],
                accumulate_grad_batches=1, devices=self.gpus, strategy="ddp",
                num_nodes=1, callbacks=[checkpoint_callback], logger=logger,
                log_every_n_steps=1,max_epochs=train_params['params']['epochs']
                )
            ...
            for rank in range(n_processes):
                process = mp.Process(target=trainer.fit(trainee, train_loader,
                    val_loader), args=(segnet,)
                    )
                process.start()
